...
| DEF-04 | Dependencia `AutoMapper` 14.0.0 reportada como vulnerable (High) | Application / transitivos | Alta | Riesgo de DoS por recursión profunda en mapeos (`GHSA-rvv3-g6hj-g44x`) | Versión 14.0.0 | Actualizado a **15.1.1** (+ Extensions 10.0.0; ajuste `AddAutoMapper`) | `dotnet list --vulnerable` post-fix sin hallazgos; build/test OK | Corregido |
| DEF-05 | Dependencia transitiva `Microsoft.SemanticKernel.Core` 1.47.0 reportada como Critical | Infrastructure / Api / Worker | Crítica | Advisory Arbitrary File Write (`GHSA-2ww3-72rp-wpp4`) | Semantic Kernel 1.47.0 | Actualizado metapaquete a **1.71.0** (Core 1.71.0) | Segunda auditoría NuGet sin hallazgos | Corregido |
